...
namespace GateSale.API.Controllers
{
    ...
    public class DisputeController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetDisputeByOrderId(Guid orderId)
        {
            try
            {
                ...
                if (dispute == null)
                {
                    return NotFound($"No dispute found for order {orderId}");
                }
                ...
                var result = new DisputeDetailDto
                {
                    Id = dispute.Id,
                    ...
                    IsReturnRequested = dispute.IsReturnRequested,
                    IsReturnPaidBySeller = dispute.IsReturnPaidBySeller,
                    IsReturnCompleted = dispute.IsReturnCompleted,
            ...
            {
                _logger.LogError(ex, "Error retrieving dispute for order {OrderId}", orderId);
                return StatusCode(500, "An error occurred while retrieving the dispute");
            }
        }
        ...
        public async Task<IActionResult> CreateDispute(Guid orderId, [FromBody] CreateDisputeRequest request)
        {
            try
            {
                ...
                var allowedStatuses = new[] { OrderStatus.Delivered, OrderStatus.Collected, OrderStatus.BuyerApproved, OrderStatus.Completed };
                if (!allowedStatuses.Contains(order.Status))
                {
                    return BadRequest($"Cannot create dispute for order in status {order.Status}. Order must be Delivered, Collected, or Approved.");
                }
