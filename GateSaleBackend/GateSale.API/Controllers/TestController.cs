...
namespace GateSale.API.Controllers
{
    ...
    public class TestController : ControllerBase
    {
        ...
        [HttpPost("seed-whitelisted-domains")]
        public async Task<IActionResult> SeedWhitelistedDomains()
        {
            try
            {
                var domainsToAdd = new[]
                {
                    new WhitelistedDomain { Domain = "gmail.com", SchoolName = "Gmail Test Users", IsActive = true },
                    new WhitelistedDomain { Domain = "yahoo.com", SchoolName = "Yahoo Test Users", IsActive = true },
                    new WhitelistedDomain { Domain = "hotmail.com", SchoolName = "Hotmail Test Users", IsActive = true },
                    new WhitelistedDomain { Domain = "outlook.com", SchoolName = "Outlook Test Users", IsActive = true },
                    new WhitelistedDomain { Domain = "test.edu", SchoolName = "Test University", IsActive = true }
                };

                var addedDomains = new List<string>();
                
                foreach (var domain in domainsToAdd)
                {
                    var exists = await _dbContext.WhitelistedDomains.AnyAsync(w => w.Domain == domain.Domain);
                    if (!exists)
                    {
                        _dbContext.WhitelistedDomains.Add(domain);
                        addedDomains.Add(domain.Domain);
                    }
                }
                ...
                return Ok(new
                {
                    ...
                    TotalDomainsInDb = await _dbContext.WhitelistedDomains.CountAsync()
                });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error seeding whitelisted domains");
                return StatusCode(500, "An error occurred while seeding domains");
            }
        }
    }
}