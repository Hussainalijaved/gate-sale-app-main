...
namespace GateSale.API.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetBuyerOrders()
        {
            ...
            {
                var buyerId = await GetCurrentUserId();
                _logger.LogInformation("Retrieving buyer orders for BuyerId: {BuyerId}", buyerId);
                var orders = await _orderService.GetOrdersByBuyerIdAsync(buyerId);
                _logger.LogInformation("Found {Count} buyer orders", orders.Count());
        ...
        public async Task<IActionResult> GetSellerOrders()
        {
            try
            {
                ...
                _logger.LogInformation("Retrieving seller orders for SellerId: {SellerId}", sellerId);
                var orders = await _orderService.GetOrdersBySellerIdAsync(sellerId);
                _logger.LogInformation("Found {Count} seller orders", orders.Count());
