...
namespace GateSale.Infrastructure.Services
{
    public class CognitoService : ICognitoService
    {
        ...
        public async Task<bool> AdminConfirmSignUpAsync(string email)
        {
            try
            {
                ...
                _logger.LogInformation("Admin confirming sign up for {Email}", email);
                var response = await _cognitoClient.AdminConfirmSignUpAsync(request);
                return response.HttpStatusCode == HttpStatusCode.OK;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error admin confirming sign up for {Email}", email);
                throw new ApplicationException("An error occurred confirming the user account.");
            }
        }

        public async Task<bool> AdminSetUserPasswordAsync(string email, string newPassword)
        {
            try
            {
                ...
                _logger.LogInformation("Admin setting password for {Email}", email);
                var response = await _cognitoClient.AdminSetUserPasswordAsync(request);
                return response.HttpStatusCode == HttpStatusCode.OK;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error admin setting password for {Email}", email);
                throw new ApplicationException("An error occurred updating the password.");
            }
        }
        ...
        public async Task<string?> GetUserIdByEmailAsync(string email)
        {
            try
            {
                ...
                var response = await _cognitoClient.AdminGetUserAsync(request);
                return response.Username; // This is the UserSub/UserId in Cognito
            }
            catch (UserNotFoundException)
            {
                return null;
