    ...
    public class PudoLockerService : IPudoLockerService
    {
        ...
        public async Task<IEnumerable<Locker>> GetAvailableLockers(double latitude, double longitude, double radiusInKm)
        {
            try
            {
                ...
                {
                    var existing = await _dbContext.Lockers.FirstOrDefaultAsync(l => l.LockerCode == locker.LockerCode);
                    if (existing == null)
                    {
                    ...
                    {
                        // Update existing locker info if needed
                        existing.Location = locker.Location;
                        existing.Description = locker.Description;
                        existing.Latitude = locker.Latitude;
                        existing.Longitude = locker.Longitude;
