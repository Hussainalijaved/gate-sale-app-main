...
namespace GateSale.Infrastructure.Services
{
    public class OzowPaymentService : IPaymentService
    {
            ...
            IOrderService orderService)
        {
            ...
            _apiBaseUrl = (_configuration["Ozow:ApiBaseUrl"] ?? throw new ArgumentNullException("Ozow:ApiBaseUrl")).TrimEnd('/');
        ...
        public async Task<PaymentResultDto> InitiatePaymentAsync(Guid orderId, decimal amount, string returnUrl, string cancelUrl)
        {
            try
            {
                ...
                // Calculate hash signature - all fields in specific order per Ozow docs, then lowercase
                var bankRef = order.OrderNumber.Length > 20 ? order.OrderNumber.Substring(0, 20) : order.OrderNumber;
                var notifyUrl = _configuration["Ozow:DefaultNotifyUrl"] ?? "";
                var isTestStr = _testMode ? "true" : "false";
                
                // Order: SiteCode + CountryCode + CurrencyCode + Amount + TransactionReference + BankReference + Customer + CancelUrl + ErrorUrl + SuccessUrl + NotifyUrl + IsTest + PrivateKey
                var customerName = order.Buyer.FullName?.Trim() ?? "";
                var hashStringBeforeLower = $"{_siteCode}ZAZAR{amount.ToString("F2")}{paymentReference}{bankRef}{customerName}{cancelUrl}{cancelUrl}{returnUrl}{notifyUrl}{isTestStr}{_privateKey}";
                _logger.LogInformation("Hash input (before lowercase): {HashInput}", hashStringBeforeLower);
                var hashString = hashStringBeforeLower.ToLower();
                _logger.LogInformation("Hash input (after lowercase): {HashInput}", hashString);
                ...
                // Build payment URL for browser redirect (not API call)
                var queryParams = new Dictionary<string, string>
                {
                    ...
                    { "NotifyUrl", notifyUrl },
                    { "IsTest", _testMode.ToString().ToLower() },
                    { "HashCheck", hash }
                };
                
                var queryString = string.Join("&", queryParams.Select(kvp => 
                    $"{Uri.EscapeDataString(kvp.Key)}={Uri.EscapeDataString(kvp.Value)}"));
                var paymentUrl = $"https://pay.ozow.com?{queryString}";
                
                _logger.LogInformation("Generated Ozow payment URL: {PaymentUrl}", paymentUrl);
                
                // Check if transaction already exists for this order, update instead of creating new
                var existingTransaction = await _dbContext.Transactions
                    .FirstOrDefaultAsync(t => t.OrderId == orderId);
                    
                if (existingTransaction != null)
                {
                    existingTransaction.TransactionId = paymentReference;
        ...
        public async Task<RefundResultDto> ProcessRefundAsync(Guid orderId, decimal amount, string reason)
        {
            try
            {
                ...
                // Call Ozow API
                var requestUrl = $"{_apiBaseUrl}/postrefund";
                _logger.LogInformation("Initiating Ozow refund request to: {RequestUrl}", requestUrl);

                using var request = new HttpRequestMessage(HttpMethod.Post, requestUrl);
                request.Headers.Add("ApiKey", _apiKey);
                request.Headers.Accept.Add(new System.Net.Http.Headers.MediaTypeWithQualityHeaderValue("application/json"));
                request.Content = JsonContent.Create(finalRequest);
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    _logger.LogError("Ozow API error (Status: {StatusCode}): {ErrorContent}", response.StatusCode, errorContent);
                    
                    return new RefundResultDto
                    {
                        Success = false,
                        Message = $"Payment gateway error: {response.StatusCode}. {errorContent}"
                    };
                }
        ...
        public async Task<bool> VerifyPaymentStatusAsync(string paymentReference)
        {
            try
            {
                ...
                // Call Ozow API
                var requestUrl = $"{_apiBaseUrl}/GetTransactionStatus";
                _logger.LogInformation("Initiating Ozow status check to: {RequestUrl}", requestUrl);
