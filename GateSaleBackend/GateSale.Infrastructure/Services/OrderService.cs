...
namespace GateSale.Infrastructure.Services
{
    public class OrderService : IOrderService
    {
        ...
        public async Task<Order> CreateOrderAsync(Guid productId, Guid buyerId, Guid buyerLockerId, decimal shippingCost)
        {
            try
            {
                ...
                var product = await _dbContext.Products
                    .Include(p => p.Seller)
                    .FirstOrDefaultAsync(p => p.Id == productId && p.Status == ProductStatus.Available);
                ...
                // Update product status - commented out for testing (allows multiple purchases)
                ...
                // Log locker assignment event
                await _orderTrackingService.LogOrderTrackingEvent(
                    order.Id,
                    "LockerAssigned",
                    $"Collection locker assigned: {buyerLocker.Location}",
                    buyerLocker.Location);

                _logger.LogInformation("Initial tracking events logged for order {OrderId}", order.Id);
        ...
        public async Task<bool> ValidateOrderForPaymentAsync(Guid productId, Guid buyerId)
        {
            try
            {
                _logger.LogInformation("Validating order for payment. ProductId: {ProductId}, BuyerId: {BuyerId}", productId, buyerId);
                ...
                if (product == null)
                {
                    _logger.LogWarning("Validation failed: Product {ProductId} not found", productId);
                    return false;
                }

                if (product.Status != ProductStatus.Available)
                {
                    _logger.LogWarning("Validation failed: Product {ProductId} status is {Status}, expected Available", productId, product.Status);
                    return false;
                }
                ...
                {
                    _logger.LogWarning("Validation failed: Buyer {BuyerId} is the seller of product {ProductId}", buyerId, productId);
                    return false;
                }

                _logger.LogInformation("Order validation successful for Product {ProductId}", productId);
                return true;
            }
            catch (Exception ex)
            {
